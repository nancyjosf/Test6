...
class LungeCounter:
                ...
                if len(imlist) != 0:
                    if imlist[24][2] >= imlist[26][2] and imlist[23][2] >= imlist[25][2]:
                        ...
                        if not self.said:
                            os.system('say -v "karen" "down"')
                            self.said = True
                    elif imlist[24][2] <= imlist[26][2] and imlist[23][2] <= imlist[25][2] and self.position == "down":
                        self.position = "up"
                        self.count += 1
                        self.status = "false"
                        print("Lunges completed:", self.count)
                        os.system('say -v "karen" "%s"' % self.count)
    ...
    def display_count(self, image):
        posit = (int(image.shape[1] / 3 - 268 / 2), int(image.shape[0] / 4 - 36 / 2))
        cv2.putText(image, 'Lunges completed: %s' % self.count, posit, 0, 1, (255, 0, 0), 3, 15)
        if self.status == 'false':
            self.rgb = (0, 0, 255)
        else:
            self.rgb = (255, 0, 0)
